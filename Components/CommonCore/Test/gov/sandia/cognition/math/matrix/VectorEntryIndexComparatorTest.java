/*
 * File:                VectorEntryIndexComparatorTest.java
 ...
 * @since 1.0
 */
public class VectorEntryIndexComparatorTest
    extends TestCase
{
    protected Random random = new Random();
    
    public VectorEntryIndexComparatorTest(
        String testName)
    {
        super(testName);
    }
    ...
    {
        TestSuite suite = new TestSuite(VectorEntryIndexComparatorTest.class);
        
        return suite;
    }

    /**
     * Test of lowestIndex method, of class gov.sandia.isrc.math.matrix.VectorEntryIndexComparator.
     */
    public void testLowestIndex()
    {
        Vector first = VectorFactory.getDefault().createUniformRandom(10, -1.0, 1.0, random);
        Vector second = VectorFactory.getDefault().createUniformRandom(10, -1.0, 1.0, random);
        VectorEntryIndexComparator instance = VectorEntryIndexComparator.INSTANCE;
        
        Iterator<VectorEntry> it1 = first.iterator();
        it1.next();
        
        Iterator<VectorEntry> it2 = first.iterator();
        it2.next();
        it2.next();
        it2.next();
        
        VectorEntry entry1 = it1.next();
        VectorEntry entry2 = it2.next();
